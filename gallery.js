(() => {
  ...
  async function fetchRepoProjects(){try{const r=await fetch('/api/projects',{cache:'no-store'});if(!r.ok)return null;const j=await r.json();return Array.isArray(j.projects)?j.projects:null}catch{return null}}
  async function renderGallery(){
    await S?.ready;
    grid.innerHTML='';
    const repoProjects=await fetchRepoProjects();
    const projects=repoProjects||(await S?.getProjectsAsync?.())||S?.getProjects?.()||[];
