...
async function saveProject(){try{await S?.ready;if(!currentProjectId)currentProjectId=newId();const now=new Date().toISOString(),repoOld=await apiGetProject(currentProjectId),projects=(await S.getProjectsAsync?.())||readProjects(),old=repoOld||projects.find(p=>p.id===currentProjectId);const rec={id:currentProjectId,template:'modelo2',title:'Modelo 2',fileName:'Modelo 2',preview:'assets/images/modelo2-preview.png',editor:`modelo2-editor.html?project=${encodeURIComponent(currentProjectId)}`,savedAt:old?.savedAt||now,updatedAt:now,state:clone(state)};await apiSaveProject(rec);const i=projects.findIndex(p=>p.id===currentProjectId);if(i>=0)projects[i]=rec;else projects.unshift(rec);if(S.setProjectsAsync)await S.setProjectsAsync(projects);else S.setProjects(projects);if(S.setAsync)await S.setAsync(projectKey(currentProjectId),clone(state));else S.set(projectKey(currentProjectId),clone(state));history.replaceState({},'',`modelo2-editor.html?project=${encodeURIComponent(currentProjectId)}`);flash(els.saveProject,'Salvo ✓');feedback('Projeto salvo em projects/ e disponível no index.');return currentProjectId}catch(e){console.error(e);feedback('Erro ao salvar projeto: '+e.message);return null}}
async function duplicateProject(){const source=currentProjectId||await saveProject();if(!source)return;const id=newId(),now=new Date().toISOString(),copy=clone(state),projects=(await S.getProjectsAsync?.())||readProjects();const rec={id,template:'modelo2',title:'Modelo 2',fileName:'Modelo 2',preview:'assets/images/modelo2-preview.png',editor:`modelo2-editor.html?project=${encodeURIComponent(id)}`,savedAt:now,updatedAt:now,duplicatedFrom:source,state:copy};await apiSaveProject(rec);projects.unshift(rec);if(S.setProjectsAsync)await S.setProjectsAsync(projects);else S.setProjects(projects);if(S.setAsync)await S.setAsync(projectKey(id),copy);else S.set(projectKey(id),copy);currentProjectId=id;history.replaceState({},'',`modelo2-editor.html?project=${encodeURIComponent(id)}`);flash(els.duplicateProject,'Cópia criada ✓');feedback('Cópia criada como novo arquivo em projects/. O original foi preservado.')}
