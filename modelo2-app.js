...
const params=new URLSearchParams(location.search);let currentProjectId=params.get('project');let currentProjectTitle='Modelo 2';
...
function saveRange(key,ed){const sel=getSelection();if(!sel?.rangeCount)return;const r=sel.getRangeAt(0);if(ed.contains(r.commonAncestorContainer))savedRanges.set(String(key),{range:r.cloneRange(),node:ed,onChange:ed._richOnChange})}
function syncRichValue(key,node,onChange,rerender=true){onChange(node.innerHTML);if(rerender)commit();else{queuePersist();requestAnimationFrame(layoutFlow)}}
function applyStyle(key,prop,val,onChange){const item=savedRanges.get(String(key)),r=item?.range,node=item?.node,targetChange=item?.onChange||onChange;if(!r||r.collapsed||!node?.isConnected){feedback('Selecione um trecho do texto no modelo ou no campo de edição primeiro.');return}const span=document.createElement('span');span.style[prop]=val;try{span.appendChild(r.extractContents());r.insertNode(span);const sel=getSelection();sel.removeAllRanges();const nr=document.createRange();nr.selectNodeContents(span);sel.addRange(nr);savedRanges.set(String(key),{range:nr.cloneRange(),node,onChange:targetChange});syncRichValue(key,node,targetChange,!els.design.contains(node))}catch(e){console.error(e)}}
...
function wireEditable(ed,key,onChange,{canvas=false}={}){if(!ed)return;ed.dataset.richKey=key;ed._richOnChange=onChange;if(canvas){ed.contentEditable='true';ed.spellcheck=false;ed.classList.add('canvas-rich-editable')}['mouseup','keyup','focus'].forEach(ev=>ed.addEventListener(ev,()=>saveRange(key,ed)));ed.addEventListener('input',()=>{saveRange(key,ed);syncRichValue(key,ed,onChange,!canvas)})}
function wireRich(ed,toolbar,onChange,key){wireEditable(ed,key,onChange);toolbar.innerHTML=toolbarHTML();toolbar.querySelectorAll('.rt-swatch').forEach(b=>b.addEventListener('mousedown',e=>{e.preventDefault();applyStyle(key,'color',b.dataset.color,onChange)}));const w=toolbar.querySelector('.rt-weight'),z=toolbar.querySelector('.rt-size');w.addEventListener('pointerdown',()=>{});z.addEventListener('pointerdown',()=>{});w.onchange=()=>applyStyle(key,'fontWeight',w.value,onChange);z.onchange=()=>applyStyle(key,'fontSize',z.value,onChange)}
function wireCanvasStatic(){wireEditable(els.previewTitleMain,'titleMain',v=>state.titleMainHtml=v,{canvas:true});wireEditable(els.previewTitleHighlight,'titleHighlight',v=>state.titleHighlightHtml=v,{canvas:true});wireEditable(els.previewSubtitle,'subtitle',v=>state.subtitleHtml=v,{canvas:true});wireEditable(els.previewCta,'cta',v=>state.ctaHtml=v,{canvas:true})}

function feedback(msg){els.projectFeedback.textContent=msg}
function wireStaticRich(){wireRich(els.titleMain,document.querySelector('[data-toolbar-for="titleMain"]'),v=>state.titleMainHtml=v,'titleMain');wireRich(els.titleHighlight,document.querySelector('[data-toolbar-for="titleHighlight"]'),v=>state.titleHighlightHtml=v,'titleHighlight');wireRich(els.subtitle,document.querySelector('[data-toolbar-for="subtitle"]'),v=>state.subtitleHtml=v,'subtitle');wireRich(els.ctaText,document.querySelector('[data-toolbar-for="ctaText"]'),v=>state.ctaHtml=v,'cta')}
...
function renderListEditors(){
  ...
  state.listItems.forEach((item,index)=>{
    ...
    wireRich(te,w.querySelector('.list-toolbar'),v=>item.textHtml=v,`list:${index}`);
...
function renderPreview(){
  ...
  state.boxes.forEach(box=>{
    ...
    els.previewBoxes.appendChild(d);const idx=state.boxes.indexOf(box);wireEditable(d.querySelector('h3'),`box:${idx}:title`,v=>box.titleHtml=v,{canvas:true});wireEditable(d.querySelector('.box-body-preview'),`box:${idx}:body`,v=>box.bodyHtml=v,{canvas:true});
  });
  ...
  state.listItems.forEach(item=>{
    ...
    els.previewList.appendChild(row);const idx=state.listItems.indexOf(item);wireEditable(row.querySelector('.m2-list-text'),`list:${idx}`,v=>item.textHtml=v,{canvas:true});
  });
...
function layoutFlow(){
  const BOX_TOP=644, BOX_GAP=26, BOX_TO_CTA=44, CTA_TO_LIST=32, FOOTER_SAFE_TOP=1740, MIN_BOX_DENSITY=.90, MIN_LIST_DENSITY=.88;
  const boxes=[...els.previewBoxes.children], rows=[...els.previewList.children];
  ...
  const ctaH=Math.max(48,els.previewCta.scrollHeight||els.previewCta.offsetHeight||67);
  const listBaseHeights=rows.map(r=>Math.max(39,r.scrollHeight||r.offsetHeight||39));
  const LIST_GAP=14;
  const listNatural=listBaseHeights.reduce((a,b)=>a+b,0)+Math.max(0,rows.length-1)*LIST_GAP;
  // Reserve downstream text first. The spacing is always constant from the last box -> CTA -> list.
  const downstream=BOX_TO_CTA+ctaH+CTA_TO_LIST+listNatural;
  const boxAvailable=Math.max(220,FOOTER_SAFE_TOP-BOX_TOP-downstream);
  const naturalHeights=boxes.map(n=>n.offsetHeight||266);
  const natural=naturalHeights.reduce((a,b)=>a+b,0)+Math.max(0,boxes.length-1)*BOX_GAP;
  const density=Math.max(MIN_BOX_DENSITY,Math.min(1,boxAvailable/Math.max(1,natural)));
  boxes.forEach(n=>n.style.setProperty('--box-scale',String(density)));
  const heights=boxes.map(n=>n.offsetHeight||266*density),boxGap=BOX_GAP*density;
  let y=0;boxes.forEach((n,i)=>{n.style.top=y+'px';y+=heights[i];if(i<boxes.length-1)y+=boxGap});
  els.previewBoxes.style.height=y+'px';els.previewBoxes.dataset.scale=density.toFixed(3);
  const ctaTop=BOX_TOP+y+BOX_TO_CTA;els.previewCta.style.top=ctaTop+'px';
  const measuredCta=Math.max(ctaH,els.previewCta.scrollHeight||ctaH);
  const listTop=ctaTop+measuredCta+CTA_TO_LIST;els.previewList.style.top=listTop+'px';
  const availableList=Math.max(60,FOOTER_SAFE_TOP-listTop);
  const listDensity=listNatural?Math.max(MIN_LIST_DENSITY,Math.min(1,availableList/listNatural)):1;
  rows.forEach(r=>r.style.setProperty('--list-scale',String(listDensity)));
  const rowHeights=rows.map(r=>Math.max(39*listDensity,r.offsetHeight||39*listDensity)),listGap=LIST_GAP*listDensity;
  let ly=0;rows.forEach((r,i)=>{r.style.top=ly+'px';r.style.left='0';r.style.width='800px';ly+=rowHeights[i]+(i<rows.length-1?listGap:0)});
  const overflow=listTop+ly>FOOTER_SAFE_TOP+2 || (density<=MIN_BOX_DENSITY+.001 && natural*density>boxAvailable+2);
  ...
  if(overflow)feedback('O conteúdo atingiu a área segura. Reduza os textos/lista antes de adicionar outro box.');
  updateScale();
}
...
function newId(){return `modelo2-${Date.now().toString(36)}-${Math.random().toString(36).slice(2,8)}`}
...
async function askProjectName(mode){return await window.ProjectNameDialog?.open({title:mode==='duplicate'?'Duplicar projeto':'Salvar projeto',description:mode==='duplicate'?'Dê um nome para a nova cópia. O projeto original será preservado.':'Dê um nome para identificar este projeto no index.',value:mode==='duplicate'?`${currentProjectTitle||'Modelo 2'} - cópia`:(currentProjectTitle||'Modelo 2'),confirmText:mode==='duplicate'?'Criar cópia':'Salvar'})}
async function saveProject(){try{await S?.ready;const name=await askProjectName('save');if(!name)return null;if(!currentProjectId)currentProjectId=newId();const now=new Date().toISOString();const old=await ProjectService.get(currentProjectId);const previewData=await makeProjectSnapshot();const rec={id:currentProjectId,template:'modelo2',title:name,fileName:name,previewData,preview:old?.preview||'assets/images/modelo2-preview.png',editor:`modelo2-editor.html?project=${encodeURIComponent(currentProjectId)}`,savedAt:old?.savedAt||now,updatedAt:now,state:clone(state)};await ProjectService.save(rec);currentProjectTitle=name;if(S?.setAsync)await S.setAsync(projectKey(currentProjectId),clone(state));else S?.set?.(projectKey(currentProjectId),clone(state));history.replaceState({},'',`modelo2-editor.html?project=${encodeURIComponent(currentProjectId)}`);flash(els.saveProject,'Salvo ✓');feedback(`Projeto “${name}” salvo e disponível no index.`);return currentProjectId}catch(e){console.error(e);feedback('Erro ao salvar projeto: '+e.message);return null}}
async function duplicateProject(){try{await S?.ready;const name=await askProjectName('duplicate');if(!name)return null;const source=currentProjectId||null,id=newId(),now=new Date().toISOString(),copy=clone(state),previewData=await makeProjectSnapshot();const rec={id,template:'modelo2',title:name,fileName:name,previewData,preview:'assets/images/modelo2-preview.png',editor:`modelo2-editor.html?project=${encodeURIComponent(id)}`,savedAt:now,updatedAt:now,duplicatedFrom:source||undefined,state:copy};await ProjectService.save(rec);if(S?.setAsync)await S.setAsync(projectKey(id),copy);else S?.set?.(projectKey(id),copy);currentProjectId=id;currentProjectTitle=name;history.replaceState({},'',`modelo2-editor.html?project=${encodeURIComponent(id)}`);flash(els.duplicateProject,'Cópia criada ✓');feedback(`Cópia “${name}” criada. O original foi preservado.`);return id}catch(e){console.error(e);feedback('Erro ao duplicar projeto: '+e.message);return null}}
...
function safeName(s){return String(s||'Modelo 2').normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-zA-Z0-9_-]+/g,'-').replace(/^-+|-+$/g,'').toLowerCase()||'modelo-2'}
