...
function layoutFlow(){
  const BOX_TOP=644, BOX_GAP=26, CTA_TOP_BASE=1459, CTA_GAP=257, LIST_GAP=44, FOOTER_SAFE_TOP=1751, MIN_DENSITY=.88;
  ...
  const available=CTA_TOP_BASE-CTA_GAP-BOX_TOP;
  const needed=natural>available?available/natural:1;
  ...
  const ctaTop=Math.max(CTA_TOP_BASE,BOX_TOP+y+CTA_GAP);els.previewCta.style.top=ctaTop+'px';
  const ctaHeight=Math.max(67,els.previewCta.offsetHeight||67),listTop=ctaTop+ctaHeight+LIST_GAP;els.previewList.style.top=listTop+'px';
  const rows=[...els.previewList.children];rows.forEach(r=>{r.style.setProperty('--list-scale','1');r.style.transform='none'});
  const LIST_BASE_GAP=14,availableList=Math.max(60,FOOTER_SAFE_TOP-listTop-18);let rowHeights=rows.map(r=>Math.max(39,r.offsetHeight||39));let listNatural=rowHeights.reduce((a,b)=>a+b,0)+Math.max(0,rows.length-1)*LIST_BASE_GAP;const listDensity=listNatural?Math.max(.75,Math.min(1,availableList/listNatural)):1;rows.forEach(r=>r.style.setProperty('--list-scale',String(listDensity)));rowHeights=rows.map(r=>Math.max(39*listDensity,r.offsetHeight||39*listDensity));const listGap=LIST_BASE_GAP*listDensity;let ly=0;rows.forEach((r,i)=>{r.style.top=ly+'px';r.style.left='0';r.style.width='800px';ly+=rowHeights[i]+(i<rows.length-1?listGap:0)});
  const overflow=natural*density>available+2||listTop+ly>FOOTER_SAFE_TOP;
  els.addBox.disabled=state.boxes.length>=2||overflow;els.addBox.title=state.boxes.length>=2?'Limite de boxes atingido para preservar a legibilidade.':'';
  if(overflow)feedback('O conteúdo atingiu a área segura. Reduza os textos ou a lista antes de adicionar outro box.');updateScale();
}
