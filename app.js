(() => {
  ...
  async function loadPersistedState(){
    await S?.ready;
    if(currentProjectId){
      const project=await ProjectService.get(currentProjectId);
      if(project?.title)currentProjectTitle=project.title;
      if(project?.state)return mergeState(project.state);
      const projectDraft=await S?.getAsync?.(draftKey(),null);
      if(projectDraft)return mergeState(projectDraft);
    }
    const templateDraft=await S?.getAsync?.(TEMPLATE_DRAFT_KEY,null);
    return templateDraft?mergeState(templateDraft):initialState();
  }

  function persistState(){
    const snapshot={...state,version:5,savedAt:new Date().toISOString()};
    if(S?.setAsync)void S.setAsync(draftKey(),snapshot);else S?.set?.(draftKey(),snapshot);
    updateDocumentTitle();
    if(els.autosaveStatus){
      els.autosaveStatus.textContent=S?.persistent?'Salvo automaticamente agora.':'Alterações preservadas nesta sessão.';
      clearTimeout(saveTimer);
      saveTimer=setTimeout(()=>{els.autosaveStatus.textContent=S?.persistent?'Alterações salvas automaticamente neste navegador.':'Alterações preservadas nesta sessão.'},1300);
    }
  }
  ...
  async function saveProject(){
    try{
      ...
      const existing=await ProjectService.get(currentProjectId),previewData=await makeProjectSnapshot();
      ...
      const saved=await ProjectService.save(record);currentProjectTitle=name;
      if(S?.setAsync)await S.setAsync(`${PROJECT_DRAFT_PREFIX}${currentProjectId}`,cloneState(state));
  ...
  async function duplicateProject(){
    try{
      ...
      await ProjectService.save(record);
      if(S?.setAsync)await S.setAsync(`${PROJECT_DRAFT_PREFIX}${id}`,cloneState(state));
  ...
  async function init(){state=await loadPersistedState();syncContentControls();renderMedicineEditors();syncStyleControls();syncLayoutControls();wire();renderPreview();fitStage();updateDocumentTitle();setInterval(tickDate,60000)}
  init();
})();
