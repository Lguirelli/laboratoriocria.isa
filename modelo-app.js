...
async function apiSaveProject(record){const r=await fetch('/api/projects',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(record)});const j=await r.json().catch(()=>({}));if(!r.ok||!j.ok)throw new Error(j.error||'Não foi possível gravar o projeto na pasta projects/.');return j.project}

const $=id=>document.getElementById(id);
const els={titleMain:$('titleMain'),titleHighlight:$('titleHighlight'),titleSize:$('titleSize'),titleSizeOut:$('titleSizeOut'),titleAutoFit:$('titleAutoFit'),subtitle:$('subtitle'),bodyCopy:$('bodyCopy'),bodyIsList:$('bodyIsList'),additionalInfo:$('additionalInfo'),boxEditors:$('boxEditors'),addBox:$('addBox'),imageUpload:$('imageUpload'),imageZoom:$('imageZoom'),imageZoomOut:$('imageZoomOut'),resetImage:$('resetImage'),exportFormat:$('exportFormat'),exportFile:$('exportFile'),saveProject:$('saveProject'),duplicateProject:$('duplicateProject'),projectFeedback:$('projectFeedback'),design:$('design'),stageWrap:document.querySelector('.stage-wrap'),previewTitle:$('previewTitle'),previewTitleMain:$('previewTitleMain'),previewTitleHighlight:$('previewTitleHighlight'),previewSubtitle:$('previewSubtitle'),previewBoxes:$('previewBoxes'),lowerFlow:$('lowerFlow'),previewBodyCopy:$('previewBodyCopy'),previewAdditionalInfo:$('previewAdditionalInfo'),imageMask:$('imageMask'),previewImage:$('previewImage')};
...
function saveRange(ed){const sel=getSelection();if(!sel?.rangeCount)return;const r=sel.getRangeAt(0);if(ed.contains(r.commonAncestorContainer))savedRanges.set(ed,r.cloneRange())}
...
function syncInputs(){els.titleMain.innerHTML=state.titleMainHtml;els.titleHighlight.innerHTML=state.titleHighlightHtml;els.subtitle.innerHTML=state.subtitleHtml;els.bodyCopy.innerHTML=state.bodyHtml;els.additionalInfo.innerHTML=state.additionalInfoHtml;els.titleSize.value=state.titleSize;els.titleSizeOut.textContent=state.titleSize+'px';els.titleAutoFit.checked=state.titleAutoFit;els.bodyIsList.checked=state.bodyIsList;els.imageZoom.value=state.image.zoom;renderBoxEditors()}
...
function iconMarkup(icon){if(!icon)return `<img src="assets/modelo1/layer-14.png" alt="">`;if(icon.type==='svg')return colorizeSvg(icon.data);if(icon.type==='png')return `<span class="uploaded-icon-mask" style="--icon-mask:url('${icon.data}')"></span>`;return `<img src="${icon.src||'assets/modelo1/layer-14.png'}" alt="">`}
...
function layoutFlow(){
  ...
  boxes.forEach(n=>{n.style.transform='none';n.style.top='0px'});
  const heights=boxes.map(n=>n.offsetHeight||265);
  const baseGap=43;
  const naturalTotal=heights.reduce((a,b)=>a+b,0)+Math.max(0,boxes.length-1)*baseGap;
  const available=Math.max(220,1127-boxTop-43);
  const boxScale=naturalTotal?Math.max(.46,Math.min(1,available/naturalTotal)):1;
  let y=0;
  boxes.forEach((n,i)=>{
    n.style.top=y+'px';
    n.style.transform=`scale(${boxScale})`;
    n.style.transformOrigin='top center';
    y+=heights[i]*boxScale;
    if(i<boxes.length-1)y+=baseGap*boxScale;
  });
  els.previewBoxes.style.height=Math.max(0,y)+'px';
  els.previewBoxes.dataset.scale=boxScale.toFixed(3);
  const bottom=boxTop+y;
  const lower=Math.max(0,bottom+baseGap*boxScale-1127);
  els.lowerFlow.style.setProperty('--lower-offset',lower+'px');
  els.design.style.height='1920px';
  updateScale();
  if(lower>0)feedback('Os boxes foram compactados ao limite seguro. Revise o volume de conteúdo para preservar a leitura dentro de 1080 × 1920.');
}
